...
<body>
    <x-layout :imageData="$imageData ?? ''" :namaUser="$namaUser">
        <x-slot:content>
            <x-page-template>
                <x-slot:pageTitle>Application Detail</x-slot:pageTitle>
            </x-page-template>
                    ...
                    <div class="relative p-10 w-full max-w-5xl bg-white rounded-lg shadow-lg overflow-y-auto max-h-screen">
                        ...
                        <div id="fullWidthTabContent" class="border-t border-gray-200 dark:border-gray-600">
                            ...
                            <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="ge" role="tabpanel" aria-labelledby="ge-tab">
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="flex flex-col">
                                        ...
                                        <dd class="text-base font-semibold"><span>{{$application->app_name}}</span></dd>
                                    </div>
                                    <div class="flex flex-col">
                                        <dt class="mb-1 text-gray-500 md:text-base dark:text-gray-400">Id</dt>
                                        <dd class="text-base font-semibold"><span>{{$application->app_id}}</span></dd>
                                    </div>
                                    <div class="flex flex-col">
                                        <dt class="mb-1 text-gray-500 md:text-base dark:text-gray-400">Environment</dt>
                                        <dd class="text-base font-semibold"><span>{{ $application->environment }}</span></dd>
                                    </div>
                                    <div class="flex flex-col">
                                        <dt class="mb-1 text-gray-500 md:text-base dark:text-gray-400">URL</dt>
                                        <dd class="text-base font-semibold"><a href="{{ $application->app_url }}">{{ $application->app_url }}</a></dd>
                                    </div>
                                    <div class="flex flex-col">
                                        <dt class="mb-1 text-gray-500 md:text-base dark:text-gray-400">Image URL</dt>
                                        <dd class="text-base font-semibold"><a href="{{ $application->img_url?? '' }}">{{$application->img_url?? 'N/A' }}</a></dd>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="ei" role="tabpanel" aria-labelledby="ei-tab">
                                @if (!empty($application->mapCostCenterApplication) && $application->mapCostCenterApplication->isNotEmpty())
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    @foreach ($application->mapCostCenterApplication as $index => $mapCostCenterApplication)
                                    <div class="flex items-center space-x-3 rtl:space-x-reverse">
                                        ...
                                        <span>
                                            {{ $mapCostCenterApplication->costCenterHierarchy->cost_center_name }}
                                            <span class="font-semibold text-gray-900 dark:text-white">{{ $mapCostCenterApplication->costCenterHierarchy->cost_center }}</span>
                                        </span>
                                    </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="hidden p-4 bg-white rounded-lg dark:bg-gray-800" id="epb" role="tabpanel" aria-labelledby="epb-tab">
                                <!-- Accordion -->
                                @if (!empty($mapEmployeeApplication) && $mapEmployeeApplication->isNotEmpty()) <!-- Check if titleGrade has items -->
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">

                                    @foreach ($mapEmployeeApplication as $index => $mapEmployeeApplication)
                                    <div class="flex items-center space-x-3 rtl:space-x-reverse">
                                        <svg class="flex-shrink-0 w-3.5 h-3.5 text-green-500 dark:text-green-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5" />
                                        </svg>
                                        <span>
                                            {{ $mapEmployeeApplication->mEmployeeGeneralInfo->name }}
                                            <span class="font-semibold text-gray-900 dark:text-white">{{ $mapEmployeeApplication->mEmployeeGeneralInfo->BP  }}</span>
                                        </span>
                                    </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="flex items-center justify-end p-4 border-t">
                            <a href="{{route('application')}}" class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-6 py-3 text-center">
                                Back
                            </a>
                        </div>
                    </div>
